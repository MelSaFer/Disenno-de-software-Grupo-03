...
const Filters = () => {
  ...
  function handleChange(e) {
    ...
    //console.log("Esta es la categoria en el handleChange: ", category, " y este es el value: ", value)

    // Remover si no está seleccionada actualmente
    if (selectedCats.includes(value)) {
      setSelectedCats((prev) => prev.filter((cat) => cat !== value));
    }
    // Agregar si está seleccionada y no existe
    if (selected[value] && !selectedCats.includes(value)) {
    ...
    // // Filtrar selectedSubs para remover subcategorias de categoria
    // const updatedSubs = selectedSubs.filter((sub) => {
    //   return !categorySubs.includes(sub);
    // });
    ...
    // Agregar a search params

    //searchParams.set('filters', selectedCats);
  }
  ...
  useEffect(() => {
    //console.log("Estas son las categorias seleccionadas:", selectedCats);
        ...
        <div>
          {Object.keys(selected).map((categoryName) => {
            ...
            if (
              ...
              setSelectedCats((prev) => [...prev, category]);
            }
            ...
            return (
              <>
                <h4 className="text-center" style={{ marginTop: "5px" }}>
                  {categoryName}
                </h4>
                <ul>
                  {subs.map((sub) => (
